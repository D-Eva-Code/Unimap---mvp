...
async function CreateUserModelV(fullname, email, password){
try{
    key = crypto.randomBytes(32).toString('hex');
   const [data]= await newcp.conpool.query(SqlObject.addVendor, [fullname, email, password]);
//   return data;
...
async function CheckExistingUserV(email){
    try{
        const [data] = await newcp.conpool.query(SqlObject.loginSqlV, [email]);
        if (data.length > 0){
...
module.exports = {CreateUserModelV, CheckExistingUserV}