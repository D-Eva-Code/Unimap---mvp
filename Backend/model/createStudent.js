...
try{
   ...
   const [data]= await newcp.conpool.query(SqlObject.addStudent, [fullname, email, password]);
//   return data;
...
async function CheckExistingUserS(email){
    try{
        const [data] = await newcp.conpool.query(SqlObject.loginSqlS, [email]);
        if (data.length > 0){
...
module.exports = {CreateUserModelS, CheckExistingUserS}